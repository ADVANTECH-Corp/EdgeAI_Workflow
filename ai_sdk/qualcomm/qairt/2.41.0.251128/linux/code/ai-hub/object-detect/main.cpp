...
int main(int argc, char* argv[]) {
    ...
    while (true) {
        ...
        if (cv::getWindowProperty(windowName, cv::WND_PROP_VISIBLE) == 0.0) {
        ...
        try {
            ...
            if (rect.width <= 0) {
                std::cout << "Window closed by user (rect)." << std::endl;
                break;
            }
        } catch (...) {
            std::cout << "Window closed by user (exception)." << std::endl;
            break;
        }
        if (key == 'q' || key == 27) { // ESC
            break;
