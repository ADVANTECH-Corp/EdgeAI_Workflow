...
std::vector<std::shared_ptr<AsyncTask>> create_model_pipeline(
  ...
  //kent add 2025-08-15
  {
      auto checkgui_task = std::make_shared<CheckGuiTask>(); 
      auto async_task1 = std::dynamic_pointer_cast<AsyncTask>(checkgui_task);
      CHECK(async_task1 != nullptr)
      tasks.push_back(async_task1);
  }
